...
refgenomefa="/data/minimap_data/hg19.fa"
refgenomemmi="/data/minimap_data/hg19_20201203.mmi"
ilmncgmapfile="/data/minimap_data/hg19_HumanMethylation450_15017482_v1-2_cgmap.tsv"
...
gettargetbarcode () { # determine target barcode (needs to be adapted according to sequencing kit, here: RAD-SQK004)
	...
	for a in $barcodes; do
		...
		for b in $barcodedirlist; do
			...
			#echo $b" targetbarcode: "$a" = "$barcount
...
runf5cetc () {
	...
	# set B to 12 megabases (GPU) and 10 kreads
	f5c call-methylation -B12000000 -K10000 -b $fast5datadir/$fast5fileid-reads_sorted.bam -g $refgenomefa -r $fastqfile  > $fast5datadir/$fast5fileid-result.tsv
